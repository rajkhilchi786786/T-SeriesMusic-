...
async def lock_group(client, message: Message):
    ...
    if member.status not in ("administrator", "creator"):
        return await message.reply_text("❌ You need to be an admin to lock the group!")
...
async def unlock_group(client, message: Message):
    member = await client.get_chat_member(message.chat.id, message.from_user.id)
    if member.status not in ("administrator", "creator"):
        return await message.reply_text("❌ You need to be an admin to unlock the group!")
    permissions = ChatPermissions(
...
async def kick_user(client, message: Message):
    if not message.reply_to_message:
        return await message.reply_text("Reply to a user's message to kick them!")
    user = message.reply_to_message.from_user
...
async def slowmode(client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text("Usage: /slowmode <seconds>")
    try:
...
async def anti_link(client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text("Usage: /antilink on/off")
...
async def vc_logger(client, message: Message):
    if len(message.command) < 2:
        return await message.reply_text("Usage: /vclogger on/off")
    status = message.command[1].lower()
...
async def auto_approve(client, message: Message):
    member = await client.get_chat_member(message.chat.id, message.from_user.id)
    if member.status not in ("administrator", "creator"):
        return await message.reply_text("❌ You need to be an admin to toggle autoapprove!")
    VC_LOGGER_STATUS[message.chat.id] = VC_LOGGER_STATUS.get(message.chat.id, {})
...
async def edit_guardian(client, message: Message):
    await message.reply_text("✍️ Edit guardian activated. Future edits will be auto-deleted.")
...
async def check_pretender(client, message: Message):
    if not message.reply_to_message:
        return await message.reply_text("Reply to a user's message to check their last 5 messages.")
    user = message.reply_to_message.from_user
